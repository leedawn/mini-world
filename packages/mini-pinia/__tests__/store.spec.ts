...
describe('Store', () => {
  ...
  it('works without setting the active pinia', async () => {
    ...
    const w1 = mount(TestComponent, { global: { plugins: [pinia] } }) // 把 pinia 通过插件的形式传入
  ...
  it('can be reset', () => {
    ...
    store.$state.nested.foo = 'bar' // 不知道为什么这里没有触发定义的 set 方法
  ...
  // store 里面所有的值变成响应式的，这样修改的时候就可以侦听到
  ...
  it('should outlive components', async () => {
    ...
    const store = useStore(pinia) // store 返回的数据都是响应式的
