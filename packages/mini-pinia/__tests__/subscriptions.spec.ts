...
import { MutationType } from '../src/store'
...
describe('Subscriptions', () => {
  ...
  flushOptions.forEach(flush => {
    it('calls once inside components with flush ' + flush, async () => {
      ...
      const s1 = useStore() // 这里在后面需要将 isListening 和 isSyncListening 初始化为 true

      expect(spy1).toHaveBeenCalledTimes(0)

      s1.user = 'Edu'
      await nextTick() // 兼容 watch 刷新时机的不同情况
      await nextTick()
  ...
  // $subscribe 的回调放在数组里面，删除其中一个并不影响
  ...
  describe('multiple', () => {
    ...
    //   s1.user = 'd'
    //   expect(spy1).toHaveBeenCalledTimes(3)
    //   expect(spy2).toHaveBeenCalledTimes(6)  // 搞不懂，为什么没有触发 ??
    // })
  })
